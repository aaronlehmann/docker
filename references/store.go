...
type Store interface {
	References(id image.ID) []BoundNamed
	ReferencesByName(ref BoundNamed) []Association
	AddTag(ref BoundNamed, id image.ID, force bool) error
	AddDigest(ref BoundCanonical, id image.ID, force bool) error
	Delete(ref BoundNamed) (bool, error)
	Get(ref BoundNamed) (image.ID, error)
}
...
type store struct {
	...
	// up References.
	referencesByIDCache map[image.ID]map[string]BoundNamed
}
...
func NewReferenceStore(jsonPath string) (Store, error) {
	...
	store := &store{
		...
		referencesByIDCache: make(map[image.ID]map[string]BoundNamed),
	}
...
func (store *store) AddTag(ref BoundNamed, id image.ID, force bool) error {
	if _, isCanonical := ref.(BoundCanonical); isCanonical {
		return errors.New("refusing to create a tag with a digest reference")
	}
	return store.addReference(ref.WithDefaultTag(), id, force)
}

// AddDigest adds a digest reference to the store.
func (store *store) AddDigest(ref BoundCanonical, id image.ID, force bool) error {
	return store.addReference(ref, id, force)
}

func (store *store) addReference(ref BoundNamed, id image.ID, force bool) error {
	...
	if exists {
		...
		if digested, isDigest := ref.(BoundCanonical); isDigest {
	...
	if store.referencesByIDCache[id] == nil {
		store.referencesByIDCache[id] = make(map[string]BoundNamed)
	}
...
func (store *store) Delete(ref BoundNamed) (bool, error) {
...
// Get retrieves an item from the store by name.
func (store *store) Get(ref BoundNamed) (image.ID, error) {
	ref = ref.WithDefaultTag()
...
func (store *store) References(id image.ID) []BoundNamed {
	store.mu.RLock()
	defer store.mu.RUnlock()
...
func (store *store) ReferencesByName(ref BoundNamed) []Association {
	store.mu.RLock()
	defer store.mu.RUnlock()
	...
	for _, repository := range store.Repositories {
		for refStr, refID := range repository {
			...
			if store.referencesByIDCache[refID] == nil {
				store.referencesByIDCache[refID] = make(map[string]BoundNamed)
			}
