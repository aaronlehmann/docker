...
func TestRestore(t *testing.T) {
	...
	if len(children) != 1 {
		t.Fatal("invalid children length: %q", len(children))
	}
	if actual, expected := children[0], ID(id2); actual != expected {
		t.Fatal("invalid child for id1: expected %q, got %q", expected, actual)
	}
	...
	if err == nil {
		t.Fatal("expected search for %q to fail", invalidPattern)
	}
...
func (ls *mockLayerGetReleaser) Get(layer.ID) (layer.Layer, error) {
	return nil, nil
}
