...
func TestTransfer(t *testing.T) {
	makeXferFunc := func(id string) DoFunc {
		return func(progressChan chan<- Progress, start <-chan struct{}, inactive chan<- struct{}) Transfer {
			...
			go func() {
				for i := 0; i <= 10; i++ {
					progressChan <- Progress{ID: id, Action: "testing", Current: int64(i), Total: 10}
	...
	for i, id := range ids {
		xfers[i], watchers[i] = tm.Transfer(id, makeXferFunc(id), progressChan)
	}
		...
		return func(progressChan chan<- Progress, start <-chan struct{}, inactive chan<- struct{}) Transfer {
			...
			go func() {
				...
				for i := int64(0); ; i++ {
					...
					progressChan <- Progress{ID: id, Action: "testing", Current: i, Total: 10}
				}
			}()
			return xfer
		}
...
func TestDuplicateTransfer(t *testing.T) {
	...
	for i := range transfers {
		...
		t.xfer, t.watcher = tm.Transfer("id1", makeXferFunc("id1"), t.progressChan)
